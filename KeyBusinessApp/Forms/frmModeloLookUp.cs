...
{
    public partial class frmModeloLookUp : Form
    {
        Modelo modelo;
        string searchingMode = "";     

        public frmModeloLookUp()
        {
            InitializeComponent();
            modelo = new Modelo();
            fillComboLists();
        ...
        private void dgMarcas_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                frmProducto.idModelo             = int.Parse(dgModelo.Rows[e.RowIndex].Cells["cIdModelo"].Value.ToString());
                frmProducto .descModelo          = dgModelo.Rows[e.RowIndex].Cells["cDescModelo"].Value.ToString();
                this.Close();
            }
        }
